...
myApp.controller('MainController', ['$scope', '$transitions','$http', '$anchorScroll', '$location', '$stateParams', '$timeout', '$state', '$rootScope', '$window', function ($scope, $transitions, $http, $anchorScroll, $location, $stateParams, $timeout, $state, $rootScope, $window)  {
  ...
  $scope.affiliate = "CentralOklahoma";
  $scope.zoomLevel = 1;
  $scope.tab = 1;
  $scope.formData = {};
  $scope.loading = false;
  ...
  $scope.submitForm = function(formType){
    ...
    $scope.loading = true;
    $http.post('/sendmail', {
      from: '"ITNCentralOklahoma Web User" <donotreply@itnamerica.com>',
      to: 'itnamerica2018@gmail.com',
      subject: "ITNCentralOklahoma Contact Form Submitted",
      text: $scope.formData,
  ...
  $scope.submitFormWithPDF = function(formType){
    ...
    if (formType === 'volunteer') {
        ...
        $scope.formSubject = 'ITNCentralOklahoma - New volunteer application received';
        ...
        $scope.showPdf = true;
        $scope.formSubject = 'ITNCentralOklahoma - New membership application received';
        $scope.generateMultiPagePDF();
    } else if (formType === 'nonrider') {
        $scope.formSubject = 'ITNCentralOklahoma - Non-Rider application Form submitted';
        $scope.generatePDF();
    } 
  }
